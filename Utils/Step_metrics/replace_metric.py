...
def calculate_pseu_efficiency(item, pseu_item):
    ...
    for key in dataset_config.input_keys:
        ...
        pseu_seq_list.append(pseu_item[key])
...
def calculate_pseu_leak(item_entity_map, pseu_item):
    # 计算 原实体的出现过 & 候选实体未出现 的实体比例
    entity_map_len = len(item_entity_map)
    if entity_map_len == 0:
        return 0

    leak_count = 0
    for ori_entity, can_entity in item_entity_map.items():
        cur_item_input = pseu_item[dataset_config.input_keys[0]]
        if ori_entity in cur_item_input and can_entity not in cur_item_input:
            leak_count += 1

    return leak_count / entity_map_len
...
def calculate_metrics():
    ...
    for idx, item in enumerate(tqdm(dataset)):
        for i, replace_cache_file in enumerate(replace_cache_files):
            ...
            plx = calculate_perplexity(cur_pseu_item[dataset_config.input_keys[0]])
            plx_score_list_map[args.replace_method[i]].append(plx)
            psl = calculate_pseu_leak(generate_cache_file[idx], cur_pseu_item)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--generate_method", type=str, default="rand")  # 控制变量
    parser.add_argument("--replace_method", type=str, nargs="+", default=["direct", "prompt", "gen"])
    ...
    if args.end2end:
        generate_cache_file_dir = os.path.join(generate_cache_file_dir, "End2End")
    generate_cache_file_path = os.path.join(generate_cache_file_dir,
                                            args.detect_method + "-" + args.generate_method + ".txt")
    generate_cache_file = load_cache_file(generate_cache_file_path)

    ## replace item
    replace_cache_file_dir = os.path.join("Output", "Cache", args.dataset, "Replace")
    if args.end2end:
        replace_cache_file_dir = os.path.join(replace_cache_file_dir, "End2End")
    replace_cache_file_names = [args.detect_method + "-" + args.generate_method + "-" + replace_method + ".txt"
                                for replace_method in args.replace_method]
