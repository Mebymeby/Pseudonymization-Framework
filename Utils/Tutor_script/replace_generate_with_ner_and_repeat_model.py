...
'''
CUDA_VISIBLE_DEVICES=0 python scripts/replace_generate_with_ner_and_repeat_model.py \
--repeat_model_path /home/longzi/Workspace/Pytorch/MyNER-SAN/output/output_model/ft_repeat_replace_0.01_4w_20250201_merge \
--generate_model_path /data5/longzi/Workspace/Pytorch/Qwen/Qwen2.5-1.5B-Instruct \
--input_text_path input/eval_data/eva_method.20.test \
--input_entity_path input/eval_data/eva_method.20.entity_dict \
--output_file_prefix output/eva_method.20.ft_4w_0.01_repeat_prompt_sep_2 \
--repeat_prompt_path input/prompt/repeat.20250120.prompt \
--generate_prompt_path input/prompt/generate.20250120.prompt \
--output_text_head "Input: " --generate_error_thresold 0.0 \
--generate_type prompt --early_stop --early_stop_flags "<|im_end|>,<|endoftext|>,Output"
'''
...
def parse_args(args):
    ...
    # IO基础参数
    parser.add_argument('--repeat_model_path', type=str,
                        default="/data5/longzi/Workspace/Pytorch/Qwen/Qwen2.5-1.5B-Instruct",
                        required=False)
    parser.add_argument('--generate_model_path', type=str,
                        default="/data5/longzi/Workspace/Pytorch/Qwen/Qwen2.5-1.5B-Instruct",
                        required=False)
    ...
    parser.add_argument("--early_stop_flags", type=str, required=False, default="<|im_end|>,<|endoftext|>,Output:")
    ...
    flags.model_path = flags.repeat_model_path
    if flags.early_stop:
...
# 读取预训练好的模型
def from_pretrained(flags, model_name=None):
    # 载入模型部分
...
def read_data(flags):
    ...
    if flags.generate_type == "topk":
        ...
        # 当替换方式为prompt时
        # 如果repeat和generate采用不同的模型
        # 那此时要做不同的处理：为了节省显存消耗，先用生成模型将所有实体一并生成替换实体
        # 先载入模型
        gen_model, gen_tokenizer = from_pretrained(flags, model_name=flags.generate_model_path)
        # 读取数据
        with open(flags.input_entity_path, "r", encoding="utf8") as input:
            print("[INFO] reading input entity file: {0}".format(flags.input_entity_path), file=sys.stderr, flush=True)
            count = 0
            for line in input:
                ...
                for key in data_line.keys():
                    for replace_word in data_line[key]:
                        # 执行生成
                        generate_prompt_text = flags.generate_prompt.format(replace_word)
                        response = generate_response(generate_prompt_text, gen_model, gen_tokenizer, flags)
                        response = response.strip()
                        data_dict[replace_word] = response
                input_entity.append(data_dict)

        # 全部处理完后，处理逻辑与dict一致，所以修改generate_type
        flags.generate_type = "dict"
        flags.is_output_dict = True
        if flags.is_debug:
            print("prompt generate result:", input_entity)
    ...
    for input_text, input_entity in tqdm(input_data):
        ...
        for _ in range(output_length):
            ...
            next_token = replace_generate_per_step(prompt_text, input_text, prefix_input_text, replace_list, model,
                                                   tokenizer, flags)
            # 判断是否提前结束
            if flags.early_stop and need_early_stop(next_token, flags):
                if flags.is_debug:
...
# 判断是否进行提前退出
def need_early_stop(next_token, flags):
...
def output_results(results, input_data, tokenizer, flags):
    ...
    # output_dict为真时也输出一下吧
    if flags.generate_type != "dict" or flags.is_output_dict:
...
def process_entity_dict(result_text, input_text, input_entity, tokenizer, flags):
    ...
    if flags.generate_error_thresold > 0 and \
            (len(tokenized_input_text) > (1 + flags.generate_error_thresold) * len(tokenized_result_text) \
